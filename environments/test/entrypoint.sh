...
if ! command -v curl >/test/null 2>&1; then
    echo "📦 Installation de curl..."
    apk add --no-cache curl 2>/test/null || apt-get update && apt-get install -y curl 2>/test/null
fi

if ! command -v jq >/test/null 2>&1; then
    echo "📦 Installation de jq..."
    apk add --no-cache jq 2>/test/null || apt-get update && apt-get install -y jq 2>/test/null
fi

# Vérifier la connexion à Vault avant d'aller plus loin
echo "⏳ Attente de Vault..."
until curl -s -o /test/null -w "%{http_code}" -H "X-Vault-Token: $VAULT_TOKEN" -X GET "$VAULT_ADDR/v1/sys/health" | grep -q "200"; do
...
VAULT_RESPONSE=$(curl -s -H "X-Vault-Token: $VAULT_TOKEN" -X GET "$VAULT_ADDR/v1/secret/data/test/application" | jq .data.data)
...
export SPRING_DATASOURCE_URL=$(echo "$VAULT_RESPONSE" | jq -r '.["spring.datasource.url"] // "jdbc:postgresql://localhost:5434/default-db"')
...
export SPRING_SECURITY_USER_NAME=$(echo "$VAULT_RESPONSE" | jq -r '.["spring.security.user.name"] // "admin-default"')
